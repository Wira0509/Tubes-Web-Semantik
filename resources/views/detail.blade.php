<!DOCTYPE html>
<html lang="id" prefix="og: http://ogp.me/ns# video: http://ogp.me/ns/video# schema: http://schema.org/"
    vocab="http://schema.org/" typeof="Movie">
    ...
    <header class="sticky top-0 z-30 bg-imdb-gray border-b border-imdb-light-gray">
        <div class="container mx-auto max-w-7xl px-4 md:px-8 py-3 flex items-center gap-4">
            ...
            <a href="{{ route('film.search', request()->only(['query', 'letter', 'type', 'year', 'rated', 'genre', 'sort', 'page'])) }}"
                class="p-2 text-white hover:text-imdb-yellow transition-colors" title="Kembali ke Hasil Pencarian">
            ...
            <form action="{{ route('film.search') }}" method="GET" class="w-full md:w-[32rem] ml-auto"
                ...
                <div class="relative w-full text-gray-800 focus-within:text-gray-600">
                    ...
                    <input type="text" name="query" placeholder="Cari film lain..."
                    ...
                    <button type="submit"
                        ...
                        <svg x-show="!isSubmitting" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6"
                        ...
                        <svg x-show="isSubmitting" style="display: none;" class="animate-spin h-6 w-6 text-imdb-yellow"
    ...
    <main class="container mx-auto max-w-7xl p-4 md:p-8">
        ...
        <div class="flex flex-col md:flex-row gap-8">
            ...
            <div class="flex-shrink-0 w-full md:w-1/3 lg:w-1/4">
                <img src="{{ $film['poster'] }}" alt="{{ $film['title'] }} Poster" property="image"
                    class="w-full rounded-lg shadow-2xl border-4 border-imdb-gray">

                @if($film['rating'] !== 'N/A' && $film['rating'] != 0)
                    <div class="mt-6 p-4 bg-imdb-gray rounded-lg text-center" property="aggregateRating"
            ...
            <div class="flex-grow">
                ...
                <div class="flex flex-wrap items-center gap-4 text-sm text-gray-300 mb-6">
                    <span class="border border-gray-500 px-2 py-0.5 rounded"
                        property="contentRating">{{ $film['rated'] }}</span>
                ...
                @if(isset($film['genres']) && $film['genres'])
                    <div class="flex flex-wrap gap-2 mb-6">
                        @foreach(explode(', ', $film['genres']) as $genre)
                            <span
                                class="px-3 py-1 bg-imdb-gray hover:bg-imdb-light-gray rounded-full text-sm text-white border border-gray-600 transition-colors cursor-default"
                                property="genre">
                                {{ $genre }}
                            </span>
                        @endforeach
                        ...
                        <div class="mb-4">
                            ...
                            <div class="text-white font-medium text-lg">
                                @if(!empty($film['directors_list']))
                                    @foreach($film['directors_list'] as $director)
                                        <a href="{{ route('film.search', ['query' => $director]) }}"
                                            class="text-imdb-yellow hover:underline" property="director" typeof="Person">
                            ...
                            <div class="text-white font-medium">
                                @if(!empty($film['writers_list']))
                                    @foreach($film['writers_list'] as $writer)
                                        <a href="{{ route('film.search', ['query' => $writer]) }}"
                                            class="text-imdb-yellow hover:underline" property="author" typeof="Person">
                                ...
                                @if(!empty($film['actors_list']))
                                    <ul class="list-disc list-inside space-y-1">
                                        @foreach($film['actors_list'] as $actor)
                                            <li>
                                                <a href="{{ route('film.search', ['query' => $actor]) }}"
                                                    class="text-imdb-yellow hover:underline" property="actor" typeof="Person">
                        ...
                        <div class="space-y-3 text-sm">
                            ...
                            <div class="flex justify-between">
                                ...
                                <a href="https://www.imdb.com/title/{{ $imdbId }}/" target="_blank"
                                    class="text-imdb-yellow hover:underline flex items-center gap-1">
                                    {{ $imdbId }}
